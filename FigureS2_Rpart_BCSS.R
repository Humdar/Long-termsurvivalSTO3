...
# Tree model the tree
treefit2 <- rpart(Surv(Surv_25yr_16, CODBC25yr_16) ~ tamoxifen + Prstatus_WT + Ki67status_WT +
...
#number of events
table(bc_erp_nomissing$Tsize, bc_erp_nomissing$CODBC25yr_16)

#Kaplan-meier
km <- survfit(Surv(Surv_25yr_16,CODBC25yr_16) ~ temp1, data=bc_erp_nomissing)

gtest2 <- ggsurvplot(km, data = bc_erp_nomissing, conf.int = FALSE,
                 ...
                 palette = c("darkgreen","orange3","royalblue"),
                 #colour.palette="jco",
                 ...
                 legend.title = "",
                 #legend = c(0.2, 0.2),
                 legend.labs= c("T1a/b", "T1c and T2 treated", "T1c and T2 untreated"),
